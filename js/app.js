...
function seleccionarMoneda(moneda, cultura) {
    ...
    if (!conn || conn.readyState !== WebSocket.OPEN) {
        iniciarSocket();
        if (window.intervaloMoneda) clearInterval(window.intervaloMoneda);
        window.intervaloMoneda = setInterval(pedirTipoCambio, 60000);
    } else {
        pedirTipoCambio();
    }
    mostrarAlertaMoneda(moneda);
}
...
function calcularEstadoSituacion() {
    ...
    // Helper para celdas
    const row = (label, data, isBold = false) => {
        let classes = isBold ? "px-3 py-2 font-bold" : "px-3 py-2";
        let r = `<tr class="table-row"><td class="${classes}">${label}</td>`;
        data.forEach(v => r += `<td class="${classes}">${v.toLocaleString(culturaActual, {style:'currency', currency:monedaActual})}</td>`);
        return r + '</tr>';
    };
    
    // 1. Calculamos PASIVOS y CAPITAL primero
    ...
    const totalPasivoCapital = [];
    for(let a=0; a<5; a++) {
        totalPasivoCapital.push(cxp[a] + capSocial[a] + utilRetenida[a]);
    }

    // 2. Calculamos los OTROS ACTIVOS (Cuentas por Cobrar y Activo Fijo)
    ...
    // 3. 🔥 EL TRUCO MAESTRO: Calculamos el EFECTIVO por diferencia para que cuadre perfecto
    const efectivo = [];
    const totalActivos = [];
    for(let a=0; a<5; a++) {
        const otrosActivos = cxc[a] + activoFijo[a];
        // El efectivo es lo que falta para igualar al Pasivo+Capital
        const caja = totalPasivoCapital[a] - otrosActivos; 
        efectivo.push(Math.max(0, caja));
        totalActivos.push(caja + otrosActivos);
    }

    // 4. DIBUJAMOS LA TABLA
    html += '<tr class="bg-blue-100"><td colspan="6" class="px-3 py-2 font-bold text-blue-800">ACTIVOS</td></tr>';
    html += row('Efectivo', efectivo);
    html += row('Cuentas por Cobrar', cxc);
    html += row('Activo Fijo Neto', activoFijo);
    html += row('TOTAL ACTIVOS', totalActivos, true);
    
    html += '<tr class="bg-red-100"><td colspan="6" class="px-3 py-2 font-bold text-red-800">PASIVOS</td></tr>';
    html += row('Cuentas por Pagar', cxp);
    
    html += '<tr class="bg-green-100"><td colspan="6" class="px-3 py-2 font-bold text-green-800">CAPITAL CONTABLE</td></tr>';
    html += row('Capital Social', capSocial);
    html += row('Utilidades Retenidas', utilRetenida);
    html += row('TOTAL PASIVO + CAPITAL', totalPasivoCapital, true);
...
function nuevoProyecto() {
    ...
    // 🔥 EL ARREGLO: Reseteo exacto de la memoria sin romper la estructura
    globalData = {
        mercadoMeta: 0, porcentajeConcentracion: 0,
        penetracionMercado: [0,0,0,0,0], unidadesVenta: [0,0,0,0,0], precioUnitario: [0,0,0,0,0],
        incrementoUnidades: 0, ventasProyeccion: [0,0,0,0,0], unidadesProduccion: [0,0,0,0,0],
        materiaPrima: [], costoMP: [], consumoMP: [], comprasMP: [0,0,0,0,0],
        costoMO: [0,0,0,0,0], gastosIndirectos: [0,0,0,0,0], costoProduccion: [0,0,0,0,0],
        costoVendido: [0,0,0,0,0], costoUnitarioProduccion: [0,0,0,0,0], gastosOperacion: [0,0,0,0,0],
        diasCredito: {ventas: 0, compras: 0}, descuentoProntoPago: 0,
        inversionesActivo: [], depreciacion: [0,0,0,0,0], amortizacion: [0,0,0,0,0],
        utilidadNeta: [0,0,0,0,0], activoCirculante: [0,0,0,0,0], pasivoCirculante: [0,0,0,0,0], capital: [0,0,0,0,0]
    };
...
function guardarProyecto() {
    ...
    .then(data => {
        ...
        mostrarToast(data.message || 'Proyecto guardado correctamente', 'success');
        } else {
             // Por si el servidor contesta con un status de error
             mostrarToast(data.message || 'Error al guardar el proyecto', 'error');
        }
        cargarListaDeProyectos(); // Recarga la lista
    })
    .catch(error => {
        console.error('Error al intentar guardar:', error);
        mostrarToast('Hubo un error al guardar. Revisa la consola.', 'error');
    });
}
...
function cargarProyecto(id) {
    ...
    fetch(`api/obtener_proyecto.php?id=${id}`)
        ...
        .then(datos => {
            ...
            if (datos.ind_monto_deuda && document.getElementById('ind-monto-deuda')) document.getElementById('ind-monto-deuda').value = datos.ind_monto_deuda;
            if (datos.ind_tasa_deuda && document.getElementById('ind-tasa-deuda')) document.getElementById('ind-tasa-deuda').value = datos.ind_tasa_deuda;
            if (datos.ind_tasa_socios && document.getElementById('ind-tasa-socios')) document.getElementById('ind-tasa-socios').value = datos.ind_tasa_socios;
            if (datos.ind_tasa_impuestos && document.getElementById('ind-tasa-impuestos')) document.getElementById('ind-tasa-impuestos').value = datos.ind_tasa_impuestos;
            if (datos.ind_plazo_credito && document.getElementById('ind-plazo-credito')) document.getElementById('ind-plazo-credito').value = datos.ind_plazo_credito;
            ...
            // ✨ USAMOS TU SISTEMA MODERNO DE NOTIFICACIONES ✨
            mostrarToast(`Proyecto "${datos.nombre_proyecto}" cargado correctamente.`, 'success');
        ...
        .catch(error => {
            ...
            mostrarToast('No se pudo cargar la información del proyecto.', 'error');
        });
}
...
function borrarProyecto(id) {
    ...
    .catch(error => {
        ...
        mostrarToast('Ocurrió un error al intentar eliminar el proyecto.', 'error');
    });
}
...
// --- FUNCIÓN DE CONVERSIÓN BLINDADA ---
function realizarConversion(factor) {
    console.log("🔄 Iniciando conversión matemática. Factor a multiplicar:", factor);
    
    // 1. Lista de todos los selectores donde se escribe dinero en el formulario
    ...
    // 2. Buscamos cada casilla y hacemos la magia matemática
    selectoresDinero.forEach(selector => {
        document.querySelectorAll(selector).forEach(input => {
            const valorActual = parseFloat(input.value);
            
            // 🔥 EL TRUCO: Solo convertimos si la casilla tiene un número válido y no está vacía
            if (!isNaN(valorActual) && input.value.trim() !== '') {
    ...
    console.log(`✅ Se convirtieron exitosamente ${camposConvertidos} casillas de dinero.`);

    // 3. ¡EL BLINDAJE! Usamos try-catch. Si una tabla falla, no se congela el resto del sistema.
    try {
        // Forzamos el recálculo en el orden correcto
        calcularPlan(); 
        calcularFactSensibil(); 
        calcularComprasMP();
        calcularPptoVtas();        
        ...
        calcularCostoProduccion(); 
        calcularFlujoEfectivo();
        console.log("✅ Todas las tablas visuales fueron recalculadas.");
    } catch (error) {
        console.error("❌ Error interno al intentar redibujar las tablas:", error);
    }
}
// ===========================
...
// 1. Ocultar el título gigante al hacer scroll (Arreglado el temblor)
window.addEventListener('scroll', () => {
    const recuadroTitulo = document.getElementById('recuadro-titulo');
    
    // Le decimos que se esconda al bajar más de 100px
    if (window.scrollY > 100) {
        recuadroTitulo.classList.remove('py-6', 'max-h-40', 'opacity-100');
        recuadroTitulo.classList.add('py-0', 'max-h-0', 'opacity-0');
    } 
    // Y que SOLO vuelva a aparecer si subes casi hasta el tope (menos de 20px)
...
function togglePestanas() {
    ...
    // Verificamos si el sidebar está oculto (tiene la clase -translate-x-full)
...
function mostrarToast(mensaje, tipo = 'success') {
    ...
    // 1. Fabricamos la notificación
    const toast = document.createElement('div');
    
    // 2. Elegimos los colores (Verde = éxito, Rojo = error)
    const bgClass = tipo === 'success' ? 'bg-green-500' : 'bg-red-500';
    const icono = tipo === 'success' ? '✅' : '⚠️';

    // 3. Le damos estilo moderno con Tailwind y la preparamos para animar
    toast.className = `${bgClass} text-white px-5 py-3 rounded-xl shadow-2xl flex items-center gap-3 transition-all duration-300 transform translate-y-10 opacity-0`;
    toast.innerHTML = `<span class="text-xl">${icono}</span> <span class="font-semibold text-sm">${mensaje}</span>`;
    
    // 4. La metemos a la pantalla
    contenedor.appendChild(toast);
    
    // 5. Animación de entrada (aparece deslizando)
    setTimeout(() => {
        toast.classList.remove('translate-y-10', 'opacity-0');
    }, 10);
    
    // 6. Animación de salida (desaparece después de 3 segundos)
    setTimeout(() => {
...
// Función para mostrar el cuadro gigante
function mostrarAlertaMoneda(tipoMoneda) {
    const modal = document.getElementById('modal-notificacion-moneda');
    const icono = document.getElementById('modal-icono');
    const mensaje = document.getElementById('modal-mensaje');

    // Cambiamos el texto y el dibujo según la moneda
    if (tipoMoneda === 'USD') {
        icono.textContent = '💵';
        mensaje.innerHTML = 'Valores convertidos a <br><span class="text-green-600 font-bold text-2xl">Dólares ($)</span>';
    } else if (tipoMoneda === 'EUR') {
        icono.textContent = '💶';
        mensaje.innerHTML = 'Valores convertidos a <br><span class="text-blue-600 font-bold text-2xl">Euros (€)</span>';
    } else if (tipoMoneda === 'MXN') {
        icono.textContent = '🪙'; 
        mensaje.innerHTML = 'Valores convertidos a <br><span class="text-amber-600 font-bold text-2xl">Pesos Mexicanos (MXN)</span>';
    }

    // Quitamos la clase 'hidden' para que aparezca en pantalla

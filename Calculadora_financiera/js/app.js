...
function agregarInversion() {
    ...
    const index = document.querySelectorAll('.inversion-item').length;
    
    const newRow = document.createElement('div');
    newRow.className = 'inversion-item grid grid-cols-5 gap-2 mb-2';
    newRow.innerHTML = `
        <input type="text" placeholder="Activo" class="inv-nombre text-sm" data-index="${index}">
        <input type="number" placeholder="Monto" class="inv-monto text-sm" step="0.01" data-index="${index}">
        <input type="number" placeholder="Vida útil (años)" class="inv-vida text-sm" data-index="${index}">
        <select class="inv-tipo text-sm" data-index="${index}">
            <option value="lineal">Lineal</option>
            <option value="acelerada">Acelerada</option>
        </select>
        <button onclick="calcularInversiones()" class="bg-blue-500 text-white px-2 py-1 rounded text-xs">Calcular</button>
    `;
    container.appendChild(newRow);
}
